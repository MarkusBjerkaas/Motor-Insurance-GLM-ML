...
def _metrics(prepared, multiplicity, exposure, cost):
    ...
    for name, model in prepared.items():
        ...
        result["gini"][name] = np.round(gini, 12) + 0.0  # fjerner -0.0 og avrundingsstøy
    return result
            ...
            row.update(
                {
                    ...
                    f"{metric}_lo": low,
                    f"{metric}_hi": high,
                    f"{metric}_p_better": np.nan if samples.abs().max() == 0 else (samples > 0).mean(),
                }
            )
        rows.append(row)
    ...
    return pd.DataFrame(
        {
            ...
            "P(A bedre), devians": pairwise["deviance_p_better"].map(lambda p: "–" if np.isnan(p) else f"{p:.0%}"),
            "Gini-gevinst [95 % KI]": [
                cell(r, "gini", 4) for _, r in pairwise.iterrows()
            ],
            "P(A bedre), Gini": pairwise["gini_p_better"].map(lambda p: "–" if np.isnan(p) else f"{p:.0%}"),
        }
    )
