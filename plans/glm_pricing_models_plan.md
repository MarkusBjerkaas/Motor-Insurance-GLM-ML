...
|---|---|---|
| Produkt: `policy_type` | Frekvens CC 0,176; COMP_E 0,183; COMP_N 0,692. Egenandel og dekning påvirker hvilke skader som meldes. | Obligatorisk kategorisk kontroll, COMP_E som basis. CC beholdes selv om det bare har 778 eksponeringsår og 137 skader. |
...
Anbefalingen er to faglig begrunnede utfordrere: produktets egenandels-/meldemønster kan variere med førerprofil og bilverdi. CC er lite, så **første utfordrere er bare produktspesifikke lineære helninger**, lagt til de allerede valgte felles hovedkurvene:
...
Disse er semantiske formler. Implementeringen skal lage to eksplisitte numeriske kolonner per interaksjon i notebooken: `is_cc * (x - center)` og `is_comp_n * (x - center)`, der `center` er eksponeringsvektet gjennomsnitt av x i treningsfolden etter imputasjon. Gjenbruk dette senteret i validering/prediksjon. Da tilfører hver interaksjon **to** parametere. En rå Patsy-utvidelse kan lage en redundant felles helning når hovedleddet er en spline. Behold hovedleddene (hierarki), bruk COMP_E som produktbasis, og kontroller rang mot den additive modellen.
...
- Vis produktvise effekter på felles, støttede områder, særlig CC. Ingen full produktspesifikk df=4-spline, merkeinteraksjoner, treveisledd eller automatisert søk.
...
- Modellrammen har fortsatt 56 084 rader, omtrent 37 904,44 eksponeringsår og 10 126 skader. Alle nullskadeår og de 9 nullkostnadsskadeårene inngår i frekvens.
...
| B-28 | Bare produkt × alder og produkt × logverdi, med to ekstra kontrasthelninger hver, støtteport og B-08. Hierarki beholdes; ingen automatisert interaksjonsjakt | Brukerbesluttet adgang; teknisk avgrensning i faseplan |
