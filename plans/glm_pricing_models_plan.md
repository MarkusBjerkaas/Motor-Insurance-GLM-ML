...
| 2026-09-19 | Opprydding av repoet (metoden er uendret): notebookene er nummerert `01_descriptiv`, `02_frekvens`, `03_severity`, `04_tweedie`, `05_catboost` (`model_results` beholder navnet), `.py`-speilene ligger i `py_mirrors/` (jupytext-parring `ipynb,py_mirrors//py:percent`), `bonus_score_analysis` er flyttet til `temp/` og litteraturgjennomgangen til `docs/`. CLAUDE.md, hooks og `.gitignore` er oppdatert. |
| 2026-09-19 | CatBoost er tunet på nytt med bredere rutenett (`depth` 2–6, `learning_rate` 0.01–0.05, `iterations` 150–600, `l2_leaf_reg` 1–30; 192 kombinasjoner) fordi forrige valg lå i nedre hjørne av alle dimensjoner. Kun hyperparametre som styrer kompleksitet og regularisering tunes. Beste verdier hentes fra modellobjektet i notebooken (ikke hardkodet). GLM-ene beholder klippingen til treningsområdet for rettferdig sammenligning med flate trær; dette er notert i begrensningene. |
| 2026-09-19 | 2024-evalueringen er kjørt én gang i `model_results` etter eksplisitt godkjenning fra prosjekteier, med alle modeller låst (CatBoost-retune fullført først; forrige valg lå fortsatt på gridkanten, notert som begrensning). Resultat: Tweedie-deviance 32,180 (toleddet GLM), 32,204 (Tweedie-GLM), 32,213 (CatBoost); balanse +0,9 %, −0,8 % og −4,0 %. Ingen modell er endret etter testen. Parvis bootstrap er ikke implementert. |
| 2026-09-19 | Presentasjonen av 2024-evalueringen er utvidet (ingen modell er endret, ingen ny bruk av 2024): nullmodell (flat 2022–23-rate), Gini via ordnet Lorenz-kurve, $D^2$ mot nullmodellen og parvis klyngebootstrap over `insured_id` med $B=10\,000$ (`src_model_comparison/gini_bootstrap.py`). CatBoost er også vist med en nivåfaktor $c=1{,}0273$ estimert fra OOF-prediksjoner på 2022–2023 (`recalibration.py`); dette er en post hoc-variant som ikke erstatter den forhåndsspesifiserte modellen. Resultat: alle modeller har $D^2\approx 3{,}5$ %, alle parvise devians- og Gini-forskjeller har 95 %-intervall som inkluderer 0; rekalibrering flytter CatBoost-balansen fra −4,0 % til −1,4 % uten å endre Gini eller deviance. |
