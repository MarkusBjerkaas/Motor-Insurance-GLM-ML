...
| 2026-09-19 | Implementering | Implementert `src_core_glm/residual_diagnostics.py` og integrert i frekvens (§7) og severity (§8); Tweedie er testet syntetisk, men ikke integrert. **Avvik fra planen:** (1) tester ligger i `src_asserts/residual_diagnostics_asserts.py` (15 syntetiske kontroller, `run_residual_diagnostics_checks()`) i stedet for `tests/`+unittest, etter brukerens krav; (2) den syntetiske suiten kjøres ikke i notebookene, bare `assert_residual_diagnostics_result` på ekte resultat; (3) ytre folder skjæres mot `data.index` (`_restrict_folds`) slik at severity kan bruke folder fra hele modellpopulasjonen; (4) stub-testen for CatBoost er erstattet av en SHAP-additivitetstest (SHAP-sum + base = log korreksjonsfaktor) og en lekkasjetest med spy på fit-kall og endret validerings-respons. **Feil funnet og rettet:** `better_than_depth_1_folds` ble beregnet for feil rader; nå Series-basert maske (kun dybde 3) med egen test. **Resultat:** ingen stabil gevinst i noen av modellene (frekvens: pooled OOF-deviance GLM 1,1217, dybde 1/3 −0,0002/−0,0010; severity: GLM 0,7233, dybde 1/3 −0,0003/−0,0033), så importance/SHAP-trinnet (7.2) er hoppet over og GLM-spesifikasjonene er uendret. |
