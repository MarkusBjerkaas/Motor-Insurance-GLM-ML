...
- Modellspesifikasjoner, kandidatregister, preprocessingregler, CV-definisjon og seleksjonsalgoritme ligger i `02_frekvens.py`.
...
|---|---|---|
| 2026-09-19 | Portering av seleksjonsalgoritmen fra `04_tweedie.py` (siste steg) | Severity-notebooken bruker nå samme algoritme og struktur som Tweedie: alder → bilverdi → geografi → tillegg (forover) → ablasjon (bakover) → forover-seleksjonstabell med SE → topp-3 → `summary()`, med firleddsregel (`SE_MULTIPLIER = 1.0`) og parallell evaluering. Registeret: `LOCKED` = `policy_type`, `year`; `CORE` = `municipality_type`, `log_vehicle_value`, `driver_age` (lineær); alder df 2/3/4, bilverdi df 3, ytelse (lineær og df 3); tillegg som i Tweedie; geografitest `municipality_type` → `circulation_area`. **Avvik fra §2:** de gamle enkeltutfordrerne (C1-stige, støtteport, bootstrap) er erstattet av den felles algoritmen; ingen interaksjoner (uendret utelatt i severity). Foldkontrakt (fit/score på severity-delmengden, prediksjon på hele valideringsfolden), Gamma-familie og `BASE_LEVELS` (S-06) er uendret. Valgt modell: `policy_type`, `year`, `circulation_area`, `log_vehicle_value`, `driver_age` (lineær), `business_type`; 7 parametere, OOF-deviance 0,7237 (steg 1 kjøresone i stedet for kommunetype: 0,7248, gevinst/SE 2,53; steg 2 `business_type`: gevinst/SE 1,59). Beholdt og koblet til sluttmodellen: storskadeplott (§5), relativitetstabell med KI (§6) og CatBoost-diagnostikk (§7); gamle «Tolkning»-avsnitt er fjernet fordi tallene er endret. 2024 er urørt. |
| 2026-09-19 | Tweedie-koblinger (metoden uendret) | Tweedie-notebooken gjenbruker `build_severity_inputs` (B-15-utvalget) til å utlede $p$ fra en Gamma-GLM-dispersjon (`tweedie_plan.md`, T-05); severity-modellen er uendret. Tweedie-notebooken har fått forover-seleksjon med flere tillegg (T-09). Portering til severity er planlagt først etter at Tweedie fungerer og etter egen godkjenning; planen skrives da om og logges. `add_seat_category` ligger nå i `src_core_glm/model_data.py`. |
